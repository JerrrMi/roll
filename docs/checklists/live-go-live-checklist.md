# Live 上线前检查清单（可打印 · USD-M / U 本位）

> 完整步骤与命令见 [`docs/live-go-live-acceptance.md`](../live-go-live-acceptance.md)。  
> **当前系统标准：** Binance **USD-M / U 本位 USDT 永续**（`/fapi/v1`）。网页操作在 **U 本位合约 / USD-M Futures**，不是 COIN-M。  
...
- [ ] `binance.product` 均为 `usdm`；`binance.api_prefix` 均为 `/fapi/v1`
...
- [ ] live `rest_base` = `https://fapi.binance.com`（**不是** `dapi.binance.com`）
- [ ] 配置中**无** `coin_m_prefix`、`/dapi` 或 `DOGEUSD_PERP` 等 COIN-M 旧字段
- [ ] `chmod 700 config/secrets`；`chmod 600 config/secrets/*.env`（Linux）
- [ ] live API Key：**禁止提现**；具备 **USD-M Futures** 权限；已配置服务器 IP **白名单**（若启用）
...
- [ ] [Binance Futures Testnet](https://testnet.binancefuture.com) **U 本位 / USD-M** 网页：仓位 0、无挂单
...
- [ ] 日志：候选标的足够（USD-M symbol 如 `DOGEUSDT`）、周期扫描正常、无 Secret 明文
- [ ] `[dry-run]` 决策与人工预期一致
...
- [ ] [Binance](https://www.binance.com) 实盘 **U 本位合约 / USD-M Futures** 与对账一致
- [ ] 已清理一切非预期持仓/挂单（若有）

## E. 阶段 4 — Live 首次 signed 极小资金（必做）

- [ ] 账户仅保留**可承受全部损失**的极小 USDT 保证金
...
- [ ] USD-M 合约规则（最小数量、tick、杠杆上限）与 Testnet 差异已阅读
- [ ] 服务器 NTP / 时间同步正常
- [ ] 告警与值班：谁负责停机和 **USD-M 网页**平仓
- [ ] 密钥泄漏应急预案
- [ ] 接受「不保证盈利」与最大可承受亏损
