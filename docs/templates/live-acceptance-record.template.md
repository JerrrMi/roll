...
| `binance.product=usdm`，`api_prefix=/fapi/v1` | | |
...
| `rest_base=https://fapi.binance.com`，`api_prefix=/fapi/v1` | | |
| 连续运行 ≥24 小时 | | 起止见 `dry-run-started-at.txt` |
| 候选标的 ≥ `min_monitor_symbols`（USD-M symbol） | | |
...
| 无非预期持仓（含手动遗留仓） | | |
| 无非预期挂单 | | |
| Binance 实盘 **USD-M / U 本位** 网页与对账一致 | | |

## 阶段 4：Live 首次 signed（极小资金，`--once --no-dry-run`）

| 检查项 | 结果 | 备注 |
| --- | --- | --- |
| 已采用极小 USDT 保证金 / 保守 `strategy` 参数（见 `settings.live.minimal-funds.example.yaml`） | | |
...
- [ ] 止损 / STOP 挂单在 **USD-M** 交易所可见且符合预期（若有持仓）
- [ ] 亏损与滑点在可接受范围
- [ ] 应急流程已演练：停进程 → **USD-M 网页**撤单 → 市价平仓 → 对账
